...
#include <assert.h>
#include <string.h>
/* Pull in ALL default NimBLE settings */
...
{
    ble_svc_hid_reset();
    /* ble_gatts_free_svcs() not available in upstream NimBLE */
}
...
uint16_t
ble_svc_hid_get_kbd_inp_handle(int instance)
{
    if (instance < get_curr_svc_idx()) {
        return hid_instances[instance].kbd_inp_handle;
    }
    return 0;
}

uint16_t
ble_svc_hid_get_report_handle(int instance, uint8_t id, uint8_t type)
{
    int i;
    if (instance >= get_curr_svc_idx()) return 0;
    for (i = 0; i < hid_instances[instance].rpts_len; i++) {
        if (hid_instances[instance].rpts[i].id == id &&
            hid_instances[instance].rpts[i].type == type) {
            return hid_instances[instance].rpts[i].handle;
        }
    }
    return 0;
}
...
int
ble_svc_hid_report_data_set(int instance, uint8_t id, uint8_t type,
                            const uint8_t *data, uint16_t len)
{
    int i;
    if (instance >= get_curr_svc_idx()) return -1;
    for (i = 0; i < hid_instances[instance].rpts_len; i++) {
        if (hid_instances[instance].rpts[i].id == id &&
            hid_instances[instance].rpts[i].type == type) {
            if (len > RPT_MAX_LEN) len = RPT_MAX_LEN;
            memcpy(hid_instances[instance].rpts[i].data, data, len);
            hid_instances[instance].rpts[i].len = len;
            return 0;
        }
    }
    return -1;
